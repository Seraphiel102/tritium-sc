...
class DwellTracker:
    ...
    def __init__(
        ...
        # target_id -> {anchor_x, anchor_y, anchor_time, event_id, notified}
        self._tracking: dict[str, dict] = {}

        # target_id -> DwellEvent (active dwells only)
        self._active_dwells: dict[str, DwellEvent] = {}

        # Historical completed dwells (capped at 200)
    ...
    def _check_all_targets(self) -> None:
        ...
        # Clean up tracking for targets that no longer exist
        with self._lock:
    ...
    def _check_target(
        ...
        with self._lock:
            ...
            if state is None:
                # First time seeing this target — set anchor
            ...
            if dist > self._radius_m:
                # Target moved — reset anchor, end any active dwell
            ...
            # Target is within radius of anchor
