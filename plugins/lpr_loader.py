...
re-exports LPRPlugin so it's discoverable without modifying
...
# Ensure the plugins/ directory is on sys.path so the lpr
# package can be imported.
_plugins_dir = str(Path(__file__).resolve().parent)
if _plugins_dir not in sys.path:
    sys.path.insert(0, _plugins_dir)

from lpr.plugin import LPRPlugin  # noqa: E402, F401
